...
object Formatters {
  ...
  def formatNERResult(postTitle: String, entities: List[NamedEntity]): String = {
    ...
    if (entities.isEmpty) {
      ...
      val lines = entities.map(e => s"  ${e.describe}").mkString("\n")
      s"$header\nEntidades detectadas:\n$lines"
    }
  }
  ...
  def formatEntityStats(counts: Map[String, Int]): String = {
    val sortedCount = counts.toList.sortBy(_._2)(Ordering[Int].reverse)
    val title = "=== Estadísticas de entidades === \n"
    val data = sortedCount.map { case (entity, amount) => 
          s"$entity: $amount"
      }.mkString("\n")
    title + data 
  }
